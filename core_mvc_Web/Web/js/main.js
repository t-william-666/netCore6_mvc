...
$("#orders").addClass("tab-pane fade active show");

var zonji = 0;

// 动态合计购物车总价
var dynamicTotal = $(".single-cart-item").each(function (i) {
	var ttext = $(this).find("span.cart-price").text();
	var sum = $(this).find("span.cart-sum1").text();

	var heji = parseInt(ttext) * parseInt(sum);
	zonji += heji;
	$("#totalprice").text(zonji);
})
...
$(".IDelete").click(function () {
	...
	var dynamicTotal = $(".single-cart-item").each(function (i) {
		...
		var heji = parseInt(ttext) * parseInt(sum);
		jianzon+=heji;
		$("#totalprice").text(jianzon);

		//购物车图标上的数字
		var shuliang=$(".single-cart-item").length;
		$(".cart-item_count").text(shuliang);
	})

	if($(".single-cart-item").length==0){
